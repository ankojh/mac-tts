...
Following stays attached to the original accessible document or article/response container. It skips editable chat composers and does not switch to whatever tab you open next. It queues completed chunks while text grows and releases the trailing fragment after it has settled for roughly 1.6 seconds. If the source closes, replaces the captured container, changes text already queued, or reaches the 100,000-character limit, select a fresh starting point. Apps must expose incoming text through Accessibility; clipboard/pasted text cannot be followed.
...
Speed choices are **1×, 1.1×, 1.25×, and 1.5×**, applied immediately by the native audio player. Voice changes restart the current sentence. Voice, speed, and cleanup preferences persist; reading text and generated audio do not.
...
- Source highlighting follows the current sentence/chunk when its audio starts. Hush draws a light yellow highlight over the original lines and a stronger orange highlight over the spoken word. Word timings come from the local model's phoneme durations and follow the audio player's position, including pauses, seeks, and speed changes. Long sentences are split at clause/word boundaries.
- The alignment maps contextual phonemes back to written words, including multiword pronunciations of numbers. A word whose pronunciation cannot be matched retains only the sentence highlight. These are model timings, not hand-corrected speech alignments, and very short words may pass faster than an app can return their screen bounds.
...
ONNX CPU inference avoids loading PyTorch or a multi-billion-parameter model. The model remains loaded while reading, using four CPU inference threads. Python generates one sentence ahead while Swift plays the current one; only current/next audio is retained in memory. Stop cancels pending work by terminating the private worker process.
...
The worker supports `status`, `prepare`, and `synthesize`, with a caller-provided request `id`. There is no HTTP endpoint, listening port, cloud TTS fallback, analytics, reading history, or persisted speech. Standard output carries JSON only; library diagnostics are discarded by the native app. Input is capped at 100,000 characters and model chunks at 260 characters.
